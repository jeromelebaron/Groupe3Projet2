...
public class DaoCommande implements IDaoCommande{
	...
	public List<Commande> getByClient(Client paramClient) {
		return em.createQuery("FROM Commande c WHERE c.client.id = :idClient AND c.commandeParente.id = null", Commande.class)
				.setParameter("idClient", paramClient.getId())
				.getResultList();
	}
	@Override
	...
	public List<LigneDeCommande> getLigneDeCommande(Commande paramCommande) {
		return em.createQuery("From LigneDeCommande l where l.commande.id = :paramId", LigneDeCommande.class)
				.setParameter("paramId", paramCommande.getId())
				.getResultList();
	}
	@Override
	public List<Commande> getSousCommande(Commande paramCommande) {
		List<Commande> list = em.createQuery("FROM Commande c WHERE c.commandeParente.id = :idCommandeParente AND c.client.id = :idClient", Commande.class)
				.setParameter("idClient", paramCommande.getClient().getId())
				.setParameter("idCommandeParente", paramCommande.getId())
				.getResultList();
		if(list.size() != 0){
		}
		return list;
	...
	{
		List<Commande> liste=em.createQuery("Select c FROM LigneDeCommande l , Commande c, Produit p WHERE c.id=l.commande.id AND l.produit.vendeur.id=:paramId and c.etat.id=2 GROUP BY c.id",Commande.class)
				.setParameter("paramId", c.getId())
				.getResultList();
		if(liste.size() != 0){
		}
		return liste;
