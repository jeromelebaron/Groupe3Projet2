...
public class DaoVille implements IDaoVille{
	...
	public List<Ville> getAll() {
		return em.createQuery("FROM Ville v", Ville.class)
				.setMaxResults(100)
				.getResultList();
	}
	@Override
	public List<Ville> getByCp(String paramCp) {
		return em.createQuery("From Ville v where v.cp like :paramCp", Ville.class)
				.setParameter("paramCp", "%"+paramCp+"%")
				.getResultList();
	}
	@Override
	...
	public Ville getByCpNom(String paramCp, String paramNom) {
		List<Ville> list = em.createQuery("From Ville v where v.cp like :paramCp and v.nom like :paramNom", Ville.class)
				.setParameter("paramCp", paramCp)
				.setParameter("paramNom", paramNom)
				.getResultList();
		if (list.size() == 1) {
			return list.get(0);
		}
