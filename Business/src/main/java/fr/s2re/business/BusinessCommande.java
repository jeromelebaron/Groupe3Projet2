...
public class BusinessCommande implements IBusinessCommande{
		...
		if(mapLigneCmd.size() != 0){
			if(mapLigneCmd.size() == 1){
				...
				Commande cmd = new Commande(
						genererNumeroCommande(paramClientDto), 
						total, 
						new Date(), 
						null, /*A changer lienSuivi*/
						mapLigneCmd.get(idVendeur), 
						AssembleurDto.methodeLivraisonFromMethodeLivraisonDto(paramCommandeDto.getMethodeLivraison()), /*A changer methode_livraison*/
				...
				log.info("id cmd inséré = "+cmd.getId());
				for(LigneDeCommande ldc : cmd.getLignesDeCommande()){
					ldc.setCommande(cmd);
					...
					if(pdtUpdate.getQuantite() >= ldc.getQuantite()){
						...
						log.info("ERREUR dans la verif des stock en amont !! qté demandé sup à celle en stock");
					}
				...
				Commande cmdParente = new Commande(
						genererNumeroCommande(paramClientDto),
						total, 
						new Date(), 
						null, /*A changer lienSuivi*/
	...
	public List<LigneDeCommandeDto> getLigneDeCommande(CommandeDto paramCommande) {
		...
		for (LigneDeCommandeDto l : AssembleurDto.listLigneCmdDtoFromListLigneCmd(daoCommande.getLigneDeCommande(AssembleurDto.cmdFromCmdDto(paramCommande)))) {
			list.add(l);
		}
		return list;
	}
	...
	public List<CommandeDto> getSousCommandes(CommandeDto paramCommande) {
		...
		//		log.info("apres");
		return AssembleurDto.listCmdDtoFromListCmd(list);
		/*for(Commande c : cmd.getSousCommandes()){
			log.info(c.getId());
		}*/
	}
	@Override
