...
public class BusinessProduit implements IBusinessProduit{
	...
	public List<ProduitDto> getByCategorie(CategorieDto paramCategorie) {
		List<ProduitDto> list = AssembleurDto.listPdtDtoFromListPdt(daoProduit.getByCategorie(AssembleurDto.categorieFromCategorieDto(paramCategorie)));
		return list;
	}
	@Override
	...
	public List<ImageDto> getImage(ProduitDto paramProduit) {
		if(paramProduit.getProduitParent() != null){
			...
			List<ImageDto> list = AssembleurDto.listImageDtoFromListImage(daoProduit.getImage(AssembleurDto.produitFromProduitDto(paramProduit.getProduitParent())));
			...
			return list;
			//return AssembleurDto.listImageDtoFromListImage(daoProduit.getImage(AssembleurDto.produitFromProduitDto(paramProduit.getProduitParent())));
		}else{
			List<ImageDto> list = AssembleurDto.listImageDtoFromListImage(daoProduit.getImage(AssembleurDto.produitFromProduitDto(paramProduit)));
			//si pdt n'a pas d'image on lui passe notre image not found
			if(list.size() == 0)
			{
				list.add(null);
